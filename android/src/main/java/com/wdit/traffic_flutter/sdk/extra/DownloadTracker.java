...
public class DownloadTracker {
    protected static final String TAG = Traffic.tag(DownloadTracker.class);
    ...
    public DownloadTracker(Tracker tracker) {
        this(tracker, getOurPackageInfo(tracker.getTraffic().getContext()));
    }
    ...
    public DownloadTracker(Tracker tracker, @NonNull PackageInfo packageInfo) {
        mTracker = tracker;
        mContext = tracker.getTraffic().getContext();
        mPreferences = tracker.getPreferences();
        mPackMan = tracker.getTraffic().getContext().getPackageManager();
    ...
    public void trackNewAppDownload(final TrackMe baseTrackme, @NonNull final Extra extra) {
        ...
        final Thread trackTask = new Thread(() -> {
            if (delay) try {
                ...
                //Timber.tag(ContentValues.TAG).e(e);
            }
    ...
    private void trackNewAppDownloadInternal(TrackMe baseTrackMe, @NonNull Extra extra) {
        ...
        if (referringApp != null && referringApp.equals(INSTALL_SOURCE_GOOGLE_PLAY)) {
            ...
            String referrerExtras = mTracker.getTraffic().getPreferences().getString(InstallReferrerReceiver.PREF_KEY_INSTALL_REFERRER_EXTRAS, null);
