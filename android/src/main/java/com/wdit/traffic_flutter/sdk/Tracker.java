...
import com.wdit.traffic_flutter.sdk.dispatcher.DispatchMode;
import com.wdit.traffic_flutter.sdk.dispatcher.Dispatcher;
import com.wdit.traffic_flutter.sdk.dispatcher.Packet;
import com.wdit.traffic_flutter.sdk.tools.Objects;
...
public class Tracker {
    private static final String TAG = Traffic.tag(Tracker.class);

    // Traffic default parameter values
    ...
    protected Tracker(Traffic traffic, TrackerBuilder config) {
        mTraffic = traffic;
        ...
        new LegacySettingsPorter(mTraffic).port(this);

        mOptOut = getPreferences().getBoolean(PREF_KEY_TRACKER_OPTOUT, false);

        mDispatcher = mTraffic.getDispatcherFactory().build(this);
        ...
        int[] res = mTraffic.getDeviceHelper().getResolution();
        if (res != null) resolution = String.format("%sx%s", res[0], res[1]);
        mDefaultTrackMe.set(QueryParams.SCREEN_RESOLUTION, resolution);

        mDefaultTrackMe.set(QueryParams.USER_AGENT, mTraffic.getDeviceHelper().getUserAgent());
        mDefaultTrackMe.set(QueryParams.LANGUAGE, mTraffic.getDeviceHelper().getUserLanguage());
    ...
    /**
     * Use this to disable this Tracker, e.g. if the rmsh_icon_wd_unchecked opted out of tracking.
    ...
    /**
     * @return true if Traffic is currently disabled
     */
    ...
    /**
     * Traffic will use the content of this object to fill in missing values before any transmission.
     ...
     * The Traffic backend accepts backdated events for up to 24 hours by default.
     * <p>
     * &gt;0 = limit in ms<br>
     * 0 = unlimited<br>
     ...
     * fetch this rmsh_icon_wd_unchecked ID from your system, and pass it to Traffic.
     * <p>
     ...
     *
     * @param userId passing null will delete the current rmsh_icon_wd_unchecked-id.
     */
    ...
    /**
     * @return a rmsh_icon_wd_unchecked-id string, either the one you set or the one Traffic generated for you.
     */
    ...
    private void injectBaseParams(TrackMe trackMe) {
        ...
        // https://github.com/traffic-org/traffic-sdk-android/issues/92
        ...
        if (mLastEvent == null || !Objects.equals(trackMe.get(QueryParams.USER_ID), mLastEvent.get(QueryParams.USER_ID))) {
            // https://github.com/traffic-org/traffic-sdk-android/issues/209
    ...
    public SharedPreferences getPreferences() {
        if (mPreferences == null) mPreferences = mTraffic.getTrackerPreferences(this);
        return mPreferences;
    }
